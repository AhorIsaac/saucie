...
if(isset($_GET["food_id"])) {
    ...
    $food = new Food();
    $deleteFood = $food->deleteFromLunchCart($food_id, $user_id);
    
    if($deleteFood === TRUE) {
        session_start();
        $_SESSION["delete_lunch_success"] = '
        ...
        $numCount = new Food();
        $numCart = $numCount->rowCountLunchCart( $user_id );
        if( $numCart == 0 ) {
            unset( $_SESSION["delete_lunch_success"] );
        }
        
        header("location: ../views/food-lunch-view.php");
        exit();
    }
}
