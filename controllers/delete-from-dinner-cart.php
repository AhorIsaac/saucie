...
if(isset($_GET["food_id"])) {
    ...
    $food = new Food();
    $deleteFood = $food->deleteFromDinnerCart($food_id, $user_id);
    
    if($deleteFood === TRUE) {
        session_start();
        $_SESSION["delete_dinner_success"] = '
        ...
        $numCount = new Food();
        $numCart = $numCount->rowCountDinnerCart( $user_id );
        if( $numCart == 0 ) {
            unset( $_SESSION["delete_dinner_success"] );
        }
        
        header("location: ../views/food-dinner-view.php");
        exit();
    }
}
