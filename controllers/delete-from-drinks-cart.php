...
if(isset($_GET["food_id"])) {
    ...
    $food = new Food();
    $deleteFood = $food->deleteFromDrinksCart($food_id, $user_id);
    
    if($deleteFood === TRUE) {
        session_start();
        $_SESSION["delete_drinks_success"] = '
        ...
        $numCount = new Food();
        $numCart = $numCount->rowCountDrinksCart( $user_id );
        if( $numCart == 0 ) {
            unset( $_SESSION["delete_drinks_success"] );
        }
        
        header("location: ../views/food-drinks-view.php");
        exit();
    }
}
