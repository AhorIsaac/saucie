...
function checkIfEmpty($inputName)
{
    if (empty($inputName)) {
        session_start();
        $_SESSION["staff_change_password_error"] =
...
if (isset($_POST["change_password"])) {
    ...
    $staff = new Staff();
    $validOldPassword = $staff->checkOldValidStaffPassword($oldPassEncrypt);

    if ($validOldPassword === TRUE) {
        $newpassword = md5($newpassword1);
        $staff = new Staff();
        $changePass = $staff->changeStaffPassword($newpassword, $id);

        if ($changePass == TRUE) {

            $user = new User();
            $changeUserPassword = $user->changeUserPassword($_SESSION["staff_email"], $newpassword);
            // var_dump($changeUserPassword); exit();            

            session_start();
            $_SESSION["staff_change_pass_success"] =
                ...
                <p>  Staff Password Changed Successfully! <br /> Welcome Back! </p>
            </div>';
        ...
        $_SESSION["staff_change_password_error"] =
            ...
            <p>  Staff Password not changed! </p>
        </div>';
        header("location: staff-change-password.php");
        exit();
    }
...
<body>
    <div class="container">
        ...
        <div class="text-center">
            <?php if (isset($_SESSION["staff_change_password_error"])) {
                echo $_SESSION["staff_change_password_error"];
                unset($_SESSION["staff_change_password_error"]);
            }
            ?>
        </div>

        <form method="POST" class="jumbotron rounded shadow-lg bg-light" id="change_pass_form">
            <h4 class="course-heading text-center text-info font-weight-bold pb-4">
                Change Password for <?php echo $_SESSION["staff_name"]; ?> <br />
                <img src="../public/storage/staff_profileimage/<?php echo $_SESSION["staff_profileimage"]; ?>" alt="" id="proim" />
            </h4>
            <input type="hidden" value="<?php echo $_SESSION["staff_id"]; ?>" name="id" />
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    ...
                    <a href="staff-homepage.php" class='btn btn-outline-dark' name="dont-change">
